...
class StartButton extends Button {
  override def drawContents(batch: PolygonSpriteBatch): Unit = {
    Text.mediumFont.draw(batch, "Play", location.x * screenUnit, (location.y + 2) * screenUnit)
  }

  var location: Vec2F = Vec2F( (Geometry.ScreenWidth / (2*screenUnit)) - 3,  (Geometry.ScreenHeight / (2*screenUnit)) - 1 )
